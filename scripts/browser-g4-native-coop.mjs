...
try {
  ...
  const host = await traveler('host');
  await host.fill('#s-seed-input', '8');
  ...
  await mobile.wait(
    "window.stichos.state.multiplayer.status==='online'",
    'code-only mobile join',
    35000,
  );
  await host.wait('window.stichos.state.multiplayer.peers.length===2', 'three travelers');
  const states = await Promise.all([host.state(), friend.state(), mobile.state()]);
  assert(states.every((s) => s.seed === 8 && s.worldGeneration === 4));
    ...
    JSON.stringify(
      {
        ...
        results: states.map((s) => ({
          ...
          body: s.player.bodyName,
          seed: s.seed,
          generation: s.worldGeneration,
          appearance: s.player.appearance,
          peers: s.multiplayer.peers,
        })),
        errors,
      },
      null,
