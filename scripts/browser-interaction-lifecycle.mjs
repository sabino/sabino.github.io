...
const out = process.env.VERSO_BROWSER_OUT || '.dream-loop/living-systems/interaction-after';
...
for (const [width, height] of process.env.VERSO_BROWSER_SINGLE
  ...
  try {
    ...
    await p.read("document.querySelector('[data-portrait-action=interact]').focus()");
    ...
    await interact();
    await touch('touchEnd');
    await p.read("document.querySelector('#s-dialogue-close').click()");
    assert.equal((await p.state()).dialogue, null);
    check('screen-reader-style detail0 click is accepted (synthetic accessibility fallback test)');
