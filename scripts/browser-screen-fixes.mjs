...
const out = path.join(root, '.dream-loop/screen-fixes');
fs.mkdirSync(out, { recursive: true });
const observations = process.argv.includes('--mobile-only')
...
async function mainScreens(c) {
  ...
  await c.click('#v-ai');
  await capture(c, 'ai');
  await c.click('[data-screen=ai] details summary');
...
try {
  ...
  const desktop = await traveler('desktop');
  if (!process.argv.includes('--mobile-only')) await mainScreens(desktop);
  const mobile = await traveler('mobile', url, true);
  await mainScreens(mobile);
  if (process.argv.includes('--mobile-only')) await startTheo(desktop);
  await desktop.focus();
  await desktop.page.send('Emulation.setDeviceMetricsOverride', {
    width: 1366,
    height: 768,
    deviceScaleFactor: 1,
    mobile: false,
  });
  await capture(desktop, 'laptop-world');
  await desktop.click('#s-life');
  await capture(desktop, 'laptop-life');
  await desktop.click('[data-life-tab="homes"]');
  await capture(desktop, 'laptop-homes');
  await desktop.click('#s-life-return');
} catch (error) {
