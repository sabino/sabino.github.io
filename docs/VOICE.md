...
## Phone setup and microphone recovery

Opening the game directly begins solo play. That is separate from browser microphone support: voice requires current membership in a shared planet/room on a voice-enabled world node. The toolbar’s **Voice setup** button always remains actionable. It opens a setup card explaining the current requirement and an explicit **Choose a shared planet / room** action; that action opens multiplayer setup, never joins or captures silently. HTTPS, missing Web Audio, missing capture APIs and an embedding page’s microphone Permissions Policy have distinct explanations. Capture restrictions preserve listen-only whenever playback is possible.

After joining, **Set up mic** opens separate **Listen only** and **Enable my microphone** choices. On microphone consent, AudioContext.resume and getUserMedia are initiated synchronously in the original button gesture, before waiting for authenticated voice-network setup. A granted track is immediately disabled while the socket/worklet finishes; it is stopped if setup fails, the page backgrounds, the room changes, or consent is canceled. PTT still requires a later press and server admission. This avoids placing a network round trip between the gesture and browser-gated APIs; it does not claim to bypass browser permission or certify every Safari hardware route.

Permission denial explains how to allow Microphone in the browser’s site settings and retry. Missing, busy or disconnected input devices have separate recovery messages. A successful setup shows **Return to game · microphone ready** and explains the visible **Hold to talk** / tap-to-talk control. Speaking never begins when permission is granted or the settings window closes.

The UI integration accepts `openRoomSetup()` and `closeSettings()` navigation callbacks. Neither callback is itself consent to join, listen or capture. Tests cover the capability/membership matrix, synchronous permission initiation before pending network work, cancellation, solo-entry permission prevention and late tracks after connection failure. Native phone permission/routing checks remain required.

Sources: [getUserMedia security, policy and error behavior](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia), [Chrome Web Audio gesture/resume guidance](https://developer.chrome.com/blog/web-audio-autoplay), [AudioContext.resume promise behavior](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext/resume).
