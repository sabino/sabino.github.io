...
This preparation uses a **browser wallet**, not a finished cross-device account system. Clearing its cookie loses browser access to that wallet. Account recovery, authenticated cross-device purchases, operating policies and production deployment remain work for activation; they must be settled before accepting real customers. The server is not designed for several processes writing the same JSON ledger.
