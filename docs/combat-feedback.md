...
Kinds cover anticipation, charge, melee, projectile, impact, guard, area, dash, death, loot, level, status and hostile telegraph. Optional styles distinguish thrusts, arcs and invented energy projectiles; status symbols distinguish healing and burning. New callers should provide exact shape/radius for meaningful danger zones. Existing `Drawing bow` and `Striking` effects receive conservative line/cone hints because the legacy `Effect` has no range field. A colocated explicit warning suppresses that approximation. These hints do not promise the exact legacy hit boundary.

Existing slash/arrow/ward/hurt/heal/ember effects adapt automatically. Physical tool strokes retain the dedicated tool animation and Foley already shipped. Loot completion receives a brief material-colored sparkle. Actor HP transitions produce death collapse only after a visible live-to-dead transition; loading a corpse does not replay a death. Physical body IDs bind preparation and impact to the correct actor, including remote players. Ordinary NPC cooldowns no longer falsely swing held equipment.
...
- Intensity zero or reduced motion immediately clears particles and disables camera displacement and squash/recoil. Threat geometry, projectiles, labels and charge progress remain legible. No screen-wide flashes or strobing.
