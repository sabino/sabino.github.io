# Earlier procedural anatomy lab

Historical guide for `?lab=1`. The default experience is now Destino: Stíchos, described in the main README.
...
Open **http://localhost:4173/?lab=1**. For a production build:
...
Open **http://localhost:4174/?lab=1**. The production output in `dist/` works on a static web server, including a subpath. The game uses local code and browser-synthesized audio, without runtime account or API requirements.
...
```sh
node scripts/browser-procedural-check.mjs http://127.0.0.1:CDP_PORT http://localhost:4174/?lab=1/
```

See [procedural browser QA](PROCEDURAL-QA.md) for tested flows, screenshots, measured performance, and limits. The earlier slice's [QA](QA.md), [storage checks](STORAGE-QA.md), [controller checks](GAMEPAD-QA.md), and [soak tests](ROBUSTNESS.md) apply to that implementation, not automatically to the new one.
