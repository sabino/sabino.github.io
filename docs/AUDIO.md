...
The living-systems batch replaces the music and extends semantic Foley. See [Adaptive audio](ADAPTIVE-AUDIO.md) for the current composition, surfaces, fauna calls, population-bound activity, source limits and timing evidence. The recorded-bank provenance and loading contracts below remain applicable.
...
Wind, water, birds and insects use at most four reusable recorded loops with 1.35-second exponential crossfades. Fire is intermittent and localized. The current score uses original eight-bar harmonic phrases and seven sustained instrument tables. Species calls, population-bound nonverbal activity and material-specific contact layers are described in [Adaptive audio](ADAPTIVE-AUDIO.md). The old per-pitch plucked score and ungrounded social layer have been replaced.

## Cache, limits and mobile lifecycle

The atlas decodes once and is reused by every recorded contact. `SoundBank` deduplicates requests, limits concurrent loads to 2, bounds streamed bytes to 6 MiB per bank and request/decode lifetime to 12 seconds. Failures back off 15 seconds with at most 3 attempts rather than loop silently. Decoded shapes are checked (mono, positive duration≤45 seconds, sane sample rate/length) before any compact allocation. Banks are retained at 24 kHz or less even on high-rate devices, within 24 MiB total. Music retains seven tiny reusable instrument tables; generated sound buffers and source limits are documented in Adaptive audio. Initial loading exposes diagnostics; only the latest 12 pending contact events can wait, for at most 300ms. Stale contacts are dropped, never replayed in a burst.

`AUDIO_LIMITS` preserves 48 transient sources, 12 ambient voices, 12 reserved effect slots, 96 cooldown records, 100 ms scheduler ticks, 240 ms lookahead. The new score schedules at most 16 current notes per callback including simultaneous chord voices; obsolete notes and phrases are skipped rather than caught up. Field loops reuse their source/bus until disposal. Transient nodes disconnect on completion. Score notes reuse tables without per-pitch PCM synthesis; animal PCM generation yields between bounded 1,024-sample slices. These budgets bound retained resources; a browser decoder's temporary internal allocation is implementation-dependent.
