...
- `setRun(run)` changes an existing journey's pace without replanning, losing its destination, or starting an idle controller.
...
### Movement pace and automatic travel

The persistent **Walk / Run** button chooses ordinary movement pace; **C** is its
dedicated keyboard shortcut. The preference is stored separately from a life at
`verso.movement-pace.v1`, defaults to Walk, and survives reloads. Changing it while
standing still never starts movement or drains stamina. Keyboard, direction-pad,
analog-stick, click routes, and Go home all use the same selected pace. Shift and
the optional stick-edge sprint remain temporary Run overrides. The existing
stamina limit still determines whether the body can actually run.

On desktop, pace and Travel are in the bottom control dock. On touch layouts,
they sit above the movement thumb and follow the selected attack handedness;
the joystick and direction-pad settings remain available. **Travel** opens a
small disclosure with **Auto walk / Auto run** and **Go home**. Opening this
disclosure releases previous held inputs. Starting a journey replaces Travel
with an immediate **Stop** control while pace remains accessible. Escape closes
the disclosure and returns focus to Travel. Changing pace during an automatic
journey preserves the current route and progress.

C is ignored while typing, inside forms, composing text, holding a modifier,
receiving a repeated key, or using a menu, conversation, satchel, transition,
construction preview, or the Travel disclosure. The toggle uses the existing
physical-input ownership ledger; controls are retained rather than rebuilt on
activation. Walk/Run is exposed as a named toggle with `aria-pressed` and
`aria-keyshortcuts="C"`; all new targets are at least 44 CSS pixels.

`tests/travel-ui.test.ts` covers shortcut exclusions, unavailable storage, and
temporary sprint composition. `tests/movement-pace-review.test.ts` independently
checks actual-session movement/stamina and route convergence. Native before/after
and control evidence is generated by `scripts/browser-pace-review.mjs` under
`.dream-loop/pace-controls/`; initial saves are explicitly prepared scenarios,
and all subsequent movement and menu actions use native input.

The final native pass records 33 checks at 320×568, 390×844 and 1280×900 with
no browser errors. Equal 450 ms movement presses travel approximately 1.305 tiles
walking and 2.023 running, matching the existing 1.55× multiplier. An independent
review additionally verifies mirrored joystick/direction-pad layouts at 320 px,
actual direction-pad running, focused chat-form shortcut exclusion, and desktop
900/1024 px dock fit. New dock targets measure 58×44 px on the narrow phone and
68×44 px on those narrow desktops. These are isolated Chromium viewport/input
checks, not physical iOS/Android certification.
