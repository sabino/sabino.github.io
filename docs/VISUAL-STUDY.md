# Earlier Verso visual study

Historical documentation for `?study=1`. The default game is now the procedural foundation described in the main README.
...
This playable slice adapts the supplied **Verso Game Concept.pdf**, **Verso Game Concept.docx**, and **Verso.pptx**. [The source brief](CONCEPT.md) records their shared ideas, differences, and the decisions behind this implementation.
...
Open **http://localhost:4173/?study=1**. To run the production build:
...
The preview opens at **http://localhost:4174/?study=1**. Both ports are fixed; stop another process using the port before starting the server. The production output is in `dist/` and can be served by a static web server. Use a server rather than opening `index.html` directly.
...
The script exercises the chapter through mouse, keyboard, and touch events. It writes screenshots and detailed results to the ignored `.dream-loop/qa/` directory and a readable report to [docs/QA.md](QA.md). The report states the tested viewports, results, and remaining gaps; passing the scripted route does not establish audio fidelity or every possible combat outcome.

Additional repeatable checks cover [save files and offline play](STORAGE-QA.md), [controller integration](GAMEPAD-QA.md), and [long-session simulation and host recovery](ROBUSTNESS.md). Production startup and service-worker registration were also verified under a nested `/games/verso/` URL, with every runtime asset resolving inside that path.
...
See [visual direction](DESIGN.md), [biome prompts](BIOMES.md), [sprite atlas notes](EXPLORER-ATLAS.md), [archivist atlas notes](ARCHIVIST-ATLAS.md), and [audio design](AUDIO.md) for the implementation's art and sound decisions.
